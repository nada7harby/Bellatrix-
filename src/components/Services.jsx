      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
            Our <span className="bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">Professional Services</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
          {services.map((service, index) => (
            <motion.div
              ...
              onClick={() => isMobile && setSelectedService(selectedService === index ? null : index)}
              className="cursor-pointer"
            >
              <div className={`h-full p-8 rounded-2xl transition-all duration-300 ${hoveredService === index ? 'shadow-xl' : 'shadow-md'} ${selectedService === index ? 'ring-2 ring-offset-2' : ''}`}
                style={{
                  backgroundColor: 'white',
                  borderTop: `4px solid ${service.color}`,
                  transform: hoveredService === index ? 'translateY(-10px)' : '',
                  boxShadow: hoveredService === index ? `0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px ${service.color}20` : ''
                }}
              >
                ...
                {(selectedService === index || !isMobile) && (
                  <motion.div
                    ...
                    animate={{ 
                      height: selectedService === index || hoveredService === index ? 'auto' : 0,
                      opacity: selectedService === index || hoveredService === index ? 1 : 0
                    }}
                    ...
                    <ul className="space-y-2 mb-6">
                      {service.details.map((detail, i) => (
                        <motion.li 
                          ...
                          animate={{ 
                            x: selectedService === index || hoveredService === index ? 0 : -10,
                            opacity: selectedService === index || hoveredService === index ? 1 : 0
                          }}
                          transition={{ delay: i * 0.05 + 0.2 }}
                          className="flex items-start"
                        >
                          <span className="w-1.5 h-1.5 rounded-full mt-2.5 mr-2 flex-shrink-0" 
                ...
                <button className={`flex items-center font-medium mt-4 ${selectedService === index ? 'text-gray-700' : 'text-gray-500'} hover:text-gray-900 transition-colors`}
                  style={{ color: selectedService === index ? service.color : '' }}
                >
                  Learn more <ArrowRightAlt className="ml-1 group-hover:translate-x-1 transition-transform" />
                </button>
              </div>
            </motion.div>
          ))}
        ...
        >
          <button className="px-8 py-4 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-medium rounded-xl hover:shadow-lg transition-all duration-300 hover:from-emerald-700 hover:to-teal-700 relative overflow-hidden group">
            ...
            >
              <div className="flex justify-between items-start mb-6">
                <div className={`w-16 h-16 rounded-xl flex items-center justify-center mr-4`}
                  style={{ backgroundColor: `${services[selectedService].color}20`, color: services[selectedService].color }}
                >
              ...
              <h3 className="text-2xl font-bold text-gray-900 mb-4">{services[selectedService].title}</h3>
              ...
              <div className="p-4 rounded-lg mb-8"
                style={{ backgroundColor: `${services[selectedService].color}10` }}
              >
                <div className="text-sm font-medium" style={{ color: services[selectedService].color }}>
